...
export default class chefMessage extends Component  {
...
contacts = () => { 
  ...
  db.collection("users").where('usertype','==',"Emp")
  .onSnapshot(snapshot => {   
    const toddo = []; 
    snapshot.forEach(doc => {
      toddo.push({
...
addReorder=(item)=>{
  ...
  this.props.navigation.navigate('chefaddReorder', {itemId: item});
}

sendMessage=(item)=>{
  // this.props.navigation.setParams({ name: item })
  this.props.navigation.navigate('chefSendMessage', {userrid: item});
}

viewContact=(item)=>{
  // this.props.navigation.setParams({ name: item })
  this.props.navigation.navigate('chefViewContact', {userid: item});
}


chefread=(item)=>{
  // this.props.navigation.setParams({ name: item })
  this.props.navigation.navigate('chefreply', {messageid: item});
}

chefReadsent=(item)=>{
  // this.props.navigation.setParams({ name: item })
  this.props.navigation.navigate('chefReadsent', {messageid: item});
}
...
<TouchableOpacity 
                          ...
                          onPress={ this.chefread.bind(this, item.itemId) }
                          >
...
<TouchableOpacity 
                          // onPress={ this.cancleRes.bind(this, item.itemId) }
                          onPress={ this.chefReadsent.bind(this, item.itemId) }
                          >
