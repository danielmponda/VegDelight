...
export default class waiterSendMessage extends Component  {
    ...
    constructor(props) {
      super()
        ...
        componentDidMount() {
            ...
            const { userrid } = this.props.navigation.state.params;

            this.setState({
               viewedUserid:userrid
            })

                const starsRef = firebase.storage().ref('usersDp/'+userrid+'.jpg');
          ...
          let doc = db.collection("users").doc(userrid)
          doc.onSnapshot(docSnapshot => {
                          this.setState({
                                  receiverfirstName: docSnapshot.data().firstName,
                                  receiverlastName:  docSnapshot.data().lastName,
                        ...
                        let docc = db.collection("users").doc(this.state.currentUserid)
                        docc.onSnapshot(docSnapshot => {
                                        this.setState({
                                                senderIID: doc.id,
                                                senderfirstName: docSnapshot.data().firstName,
                                                senderlastName:  docSnapshot.data().lastName,      
                                      })
                                      });   
          ...
          const dpr = this.state.dp ? (<TouchableOpacity><Image
                  ...
                  <StatusBar backgroundColor='#b3a7a9' barStyle="light-content"/>
                    <ScrollView>
                      ...
                      <View style={styles.cover}>
                          ...
                          <View style={styles.profilecontainer}>
                          { dps }
                          <Text style={styles.profileinfoName}> {this.state.receiverfirstName} {this.state.receiverlastName} </Text>
                        </View>
                      </View>
