        ...
        componentDidMount() {
          ...
          doc.onSnapshot(docSnapshot => {
                  ...
                  db.collection("reservation").where('currentUserId', '==',this.state.currentUser)
                   ...
                   var query = db.collection("reservation").where('currentUserId', '==',this.state.currentUser)
                   query = query.where('status', '==',"Approved")
                   query.onSnapshot(snapshot => {   
                    this.setState({
                      numberofappres:snapshot.size
                    })       
                    });    
                    ...
                    db.collection("reOrder").where('orderby', '==',this.state.currentUser)
                    .onSnapshot(snapshot => {   
                  ...
                  let resorder = db.collection("reservation").where('currentUserId', '==',this.state.currentUser)
                  resorder = resorder.where('status', '==',"Approved")
                  resorder = resorder.orderBy("date").limit(1)
                  resorder.onSnapshot(snapshot => {   
                    snapshot.forEach(doc => {
                      const realdatee = doc.data().date.toDate()
                     
                    this.setState({
                    nextres:moment(realdatee).format("DD-MM-YYYY")
                  })
                  });
          ...
          const dpr = this.state.dp ? (<TouchableOpacity onPress={ () => this.openPicker() }><Image style={styles.itemPhoto} source={{uri: this.state.dp}}/>
       </TouchableOpacity>) : 
                  ...
                  <StatusBar backgroundColor='#b3a7a9' barStyle="light-content"/>
                    <ScrollView>
                      ...
                      <View style={styles.cover}>
                          ...
                          onPress={() => this.props.navigation.navigate('cusEditProfile')}
                          >
...
<Text style={styles.innerresb}>  Upcoming Reservation:</Text>
<Text style={styles.innerrescc} > {this.state.nextres}</Text>
</View>
...
<Text style={styles.innerresb} > Approvide Reservations : </Text>
{/* <Text style={styles.innerresc} > {this.state.numberofappres}</Text> */}
<View style={[ (this.state.numberofappres > 0) ? styles.innerrescmore : styles.innerresczero ]}><Text style={[ (this.state.numberofappres > 0) ? styles.innerrestext : styles.innerrestextzero ]}> {this.state.numberofappres}</Text></View>
</View>
...
<Text style={styles.innerresb} > Number of Reservations: </Text>
{/* <Text style={styles.innerresc} > {this.state.numberofres}</Text> */}
<View style={[ (this.state.numberofres > 0) ? styles.innerrescmore : styles.innerresczero ]}><Text style={[ (this.state.numberofres > 0) ? styles.innerrestext : styles.innerrestextzero ]}> {this.state.numberofres}</Text></View>
</View>
          ...
          <View style={styles.logoutsec}>
                    ...
                    <TouchableOpacity style={styles.btnLoginh}
                           onPress={() => this.props.navigation.navigate('cusChangePassword')}
                          >
