...
export default class cusFeed extends Component  {
    ...
    reservation = () => { 
      ...
      let resorder = db.collection("reservation").where('currentUserId', '==',this.state.currentUserId)
...
Cancelledres = () => { 
  ...
  db.collection("Cancelledres").where('userCancelled', '==',this.state.currentUserId)
  .onSnapshot(snapshot => {   
    const todo = []; 
    snapshot.forEach(doc => {
      todo.push({
...
updateRes=(item)=>{
  ...
  this.props.navigation.navigate('cusupdateResInfo', {resid: item});
}
           
cancleRes=(resid,datee,time,numberofattendees,comments,status,reorderedItems,user,firstName,lastName)=>{
  const db = firebase.firestore();
  Alert.alert(
    '',
  ...
  render(){
    ...
    var SampleCancelledresArray = this.state.usCancelledresers
      ...
      <View key={key} style={styles.rescon}>
        ...
        <View style={styles.resconba}>
            <TouchableOpacity style={styles.btncanclea}
                   onPress={ this.cancleRes.bind(this,item.resid,item.date,item.time,item.numberofattendees,item.comments,item.status,item.reorderedItems,this.state.currentUserId,item.firstName,item.lastName) }
>
...
>
 ...
 <View>
                <TouchableOpacity style={styles.btnLoginn}
                  onPress={() => this.props.navigation.navigate('cusDetail')}
                  >
