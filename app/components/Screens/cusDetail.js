...
export default class cusDetail extends Component  {
    ...
    handleUpdateUsers = () => {
     ...
     const bookeddaycomp = bookedday.getFullYear()+'-'+(bookedday.getMonth()+1)+'-'+bookedday.getDate()
        ...
        if (bookedtimehour >= opentimehour && bookedtimehour <closetimehour){  
         if(numberofattendees > 0 && numberofattendees <= 30) {
          ...
          if(this.state.reorderedItems){
            ...
            var cityRef = db.collection('reOrder').doc()
            cityRef.set({
              orderby: this.state.reorderedItems,
