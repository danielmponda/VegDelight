...
export default class waiterMessage extends Component  {
...
contacts = () => { 
  ...
  contancts = db.collection("users").where('usertype','==',"Emp")
  .onSnapshot(snapshot => {   
    const toddo = []; 
    snapshot.forEach(doc => {
      toddo.push({
...
viewContact=(item)=>{
  // this.props.navigation.setParams({ name: item })
  this.props.navigation.navigate('waiterViewContact', {userid: item});
}


waiterread=(item)=>{
  // this.props.navigation.setParams({ name: item })
  this.props.navigation.navigate('waiterreply', {messageid: item});
}

waiterReadsent=(item)=>{
  // this.props.navigation.setParams({ name: item })
  this.props.navigation.navigate('waiterReadsent', {messageid: item});
}
...
<TouchableOpacity 
                          ...
                          onPress={ this.waiterread.bind(this, item.itemId) }
                          >
...
<TouchableOpacity 
                          // onPress={ this.cancleRes.bind(this, item.itemId) }
                          onPress={ this.waiterReadsent.bind(this, item.itemId) }
                          >
